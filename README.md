...
> [FORK_CURSOR.md](./docs/FORK_CURSOR.md) · [CURSOR_MCP_TODO.md](./docs/CURSOR_MCP_TODO.md) · [Design depth § summary](#design-philosophy-summary)
...
## Current goals & completed work（2026-06-18）

> Runbook: [CURSOR_MCP_TODO.md](./docs/CURSOR_MCP_TODO.md) · Gate order **A → B → C** is fixed.

### Visible goals（今見えているゴール）

**Immediate (Track B — unblocked after Gate A→B Pass):**

| # | Goal | Deliverable |
|---|------|-------------|
| **M1** | Durable orchestration transcript (Layer A) | `OrchestrationSessionStore` + Redis `orch:session:*` |
| **B-0** | Request-level model / effort / CoT presets | `inference-profiles.json` + TS-20 types |
| **B-1** | One transport path for Cursor + Wall-Bounce | `wall-bounce-analyzer.ts` → `src/adapters/*` (no nested MCP spawn) |
| **M2–M6** | Session continuity + legacy migration | `sessionId` on MCP/API · round events in Layer A · fold `codex-session-manager` under TS-22 |

**Then:** **Gate B→C** review → **Track C** (constitution enforced in code: 2–5 rounds, confidence ≥ 0.7, consensus ≥ 0.6).

**Still open in Track A (not blocking B):** A-2 InferenceProfile in unified MCP schemas · A-3 team MCP registration.

**Later tracks:** **E/F** — runtime catalog loader + cost-aware TaskRouter · OpenAI model ID migration in code · **C** — orchestrator merge · **D/P5+** — cache, Batch RAG, grounding (optional).

**North star:** Daily Cursor coding via **one MCP** (`analyze_claude` / `analyze_codex` / `analyze_agy`); hard multi-LLM analysis via **Wall-Bounce API** with the same adapters and Layer A memory.

### Completed implementation（今実装が終わっている作業）

| Area | Status | Notes |
|------|--------|-------|
| Fork Day 0 | ✅ | `forkProfile.yaml`, `config/fork/*`, adapter layout |
| **A-0** WSL CLI auth | ✅ 2026-06-18 | `claude` / `codex` / `agy` probes pass |
| **A-1** Unified MCP + adapters | ✅ 2026-06-18 | `techsapo-providers-mcp-server.ts`, `src/adapters/*`, resolver unit tests |
| **A-1 ops** Cursor registration + **G7** | ✅ 2026-06-18 | Connected in Cursor; all three `analyze_*` from Cursor Agent |
| Portable MCP config | ✅ | `npm run cursor-mcp:config` — Linux / WSL Remote / Windows+WSL templates |
| Codex adapter (MCP cwd) | ✅ | `--skip-git-repo-check` for non-repo working directories |
| **TS-21** model catalog | ✅ design | `config/llm-model-catalog.json` + schema; runtime loader = Track F |
| **TS-22** memory substrate | ✅ design | ADR v1.3, session schema/types, `orchestration-memory.json`; **G-MEM closed** |
| **Gate A→B** | ✅ **Pass 2026-06-18** | G1–G7 + G-MEM all Yes |

**Not done yet (explicit):** Redis `OrchestrationSessionStore` (M1) · Wall-Bounce → adapter wiring (B-1) · constitution hard-enforce in code (Track C) · legacy `gemini` spawn in `wall-bounce-analyzer.ts` (migration pending).
...
Three-layer orchestration memory — **Layer A mandatory** for Track B ([G-MEM closed 2026-06-18](./docs/CURSOR_MCP_TODO.md#memory-substrate-gate-prerequisite-for-track-b); **Gate A→B Pass 2026-06-18** — Track B unblocked):

| Layer | Role | AS-IS | To-Be |
|-------|------|-------|-------|
| **A** | `OrchestrationSession` — append-only transcript (`orch:session:*`) | In-process summaries only | Redis store + `sessionId` on MCP/API |
| **B** | `providerHandles` — CLI resume tokens (latency) | Legacy Codex-only Redis | Folded under Layer A |
| **C** | Cipher / RAG long-term retrieval | External MCP | Injected into round context |

**Design (v1.3):** per-event `ts` / optional `tsEnd` / `seq` / `eventId` (UTC); session `clientTimezone` (IANA, optional); idle TTL **7d** / max **30d** ([config/orchestration-memory.json](./config/orchestration-memory.json)).

| Artifact | Path |
|----------|------|
| ADR | [TECH_STACK_MEMORY_SUBSTRATE.md](./docs/decisions/TECH_STACK_MEMORY_SUBSTRATE.md) |
| JSON Schema | [config/schemas/orchestration-session.schema.json](./config/schemas/orchestration-session.schema.json) |
| Types | [src/types/orchestration-session.ts](./src/types/orchestration-session.ts) |
...
npm run techsapo-providers-mcp # manual start check (Ctrl+C)
```

**MCP 登録（環境ごとに生成 — パスを手書きしない）:**

```bash
npm run build
npm run cursor-mcp:config              # EC2 / WSL Remote / Linux → .cursor/mcp.json
npm run cursor-mcp:config -- --variant windows-wsl --print   # Windows Cursor → Settings に貼り付け
```

詳細: [docs/CURSOR_MCP_TEMPLATE.md](./docs/CURSOR_MCP_TEMPLATE.md)
...
> What we **fix in design** vs what remains open — not an implementation checklist ([runbook](./docs/CURSOR_MCP_TODO.md)).
...
|------|---------------|-----------------|
| Unified MCP + adapters | **Implemented** + G7 pass | Daily smoke + A-2/A-3 (Track A tail) |
...
| **記憶基盤（TS-22）** | Layer A 必須 transcript · UTC イベント時刻 · idle 7d / max 30d · 単一 `OrchestrationSessionStore` | [TECH_STACK_MEMORY_SUBSTRATE.md](./docs/decisions/TECH_STACK_MEMORY_SUBSTRATE.md) |
...
| 優先 | 焦点 | Track |
|------|-------------------------|-------|
| **P0** | Cursor から AI ツールを**実際に使える**状態にする | A — **Gate A→B Pass**（G7 完了） |
| **P1** | 分析 API も同じ adapter で統一 · モデル preset 本番化 · Layer A 記憶 | **B** ← **現在** |
