...
## 今見えているゴール & 実装完了済み（2026-06-18）

> 手順・ゲート: [CURSOR_MCP_TODO.md](./docs/CURSOR_MCP_TODO.md) · ゲート順 **A → B → C** 固定。

### 今見えているゴール

**直近（Track B — Gate A→B 通過により着手可能）:**

| # | ゴール | 成果物 |
|---|--------|--------|
| **M1** | オーケストレーション transcript（Layer A）の永続化 | `OrchestrationSessionStore` + Redis `orch:session:*` |
| **B-0** | リクエスト単位の model / effort / CoT preset | `inference-profiles.json` + TS-20 型 |
| **B-1** | Cursor と Wall-Bounce で同一 transport | `wall-bounce-analyzer.ts` → `src/adapters/*`（ネスト MCP spawn 廃止） |
| **M2–M6** | セッション継続 + レガシー統合 | MCP/API の `sessionId` · ラウンドイベントを Layer A へ · `codex-session-manager` を TS-22 配下へ |

**その次:** **Gate B→C** レビュー → **Track C**（憲法のコード enforce: 2–5 ラウンド、confidence ≥ 0.7、consensus ≥ 0.6）。

**Track A 残（B をブロックしない）:** A-2 統一 MCP スキーマへの InferenceProfile · A-3 チーム MCP 登録。

**以降:** **E/F** — runtime カタログ loader + コスト aware TaskRouter · コード内 OpenAI モデル ID 移行 · **C** — オーケストレータ統合 · **D/P5+** — キャッシュ、Batch RAG、Grounding（任意）。

**到達像:** 日常の Cursor コーディングは **単一 MCP**（`analyze_claude` / `analyze_codex` / `analyze_agy`）。厳密なマルチ LLM 分析は **Wall-Bounce API** — 同一 adapter + Layer A 記憶。

### 今実装が終わっている作業

| 領域 | 状態 | メモ |
|------|------|------|
| Fork Day 0 | ✅ | `forkProfile.yaml`、`config/fork/*`、adapter 配置 |
| **A-0** WSL CLI 認証 | ✅ 2026-06-18 | `claude` / `codex` / `agy` プローブ通過 |
| **A-1** 統一 MCP + adapter | ✅ 2026-06-18 | `techsapo-providers-mcp-server.ts`、`src/adapters/*`、resolver 単体テスト |
| **A-1 ops** Cursor 登録 + **G7** | ✅ 2026-06-18 | Cursor Connected；Cursor Agent から `analyze_*` 3 種 OK |
| ポータブル MCP 設定 | ✅ | `npm run cursor-mcp:config` — Linux / WSL Remote / Windows+WSL |
| Codex adapter（MCP cwd） | ✅ | 非 repo cwd 向け `--skip-git-repo-check` |
| **TS-21** モデルカタログ | ✅ 設計 | `config/llm-model-catalog.json` + schema；runtime loader は Track F |
| **TS-22** 記憶基盤 | ✅ 設計 | ADR v1.3、セッション schema/型、`orchestration-memory.json`；**G-MEM 完了** |
| **Gate A→B** | ✅ **Pass 2026-06-18** | G1–G7 + G-MEM すべて Yes |

**未完了（明示）:** Redis `OrchestrationSessionStore`（M1）· Wall-Bounce → adapter 配線（B-1）· 憲法のコード enforce（Track C）· `wall-bounce-analyzer.ts` の legacy `gemini` spawn（移行待ち）。
...
オーケストレーション記憶の 3 層 — **Layer A は Track B で必須**（[G-MEM 完了 2026-06-18](./docs/CURSOR_MCP_TODO.md#memory-substrate-gate-prerequisite-for-track-b)；**Gate A→B Pass 2026-06-18** — Track B 着手可能）:

| 層 | 役割 | AS-IS | To-Be |
|----|------|-------|-------|
| **A** | `OrchestrationSession` — append-only transcript（`orch:session:*`） | プロセス内要約のみ | Redis + MCP/API の `sessionId` |
| **B** | `providerHandles` — CLI resume（レイテンシ最適化） | Codex 専用 Redis レガシー | Layer A 配下に統合 |
| **C** | Cipher / RAG 長期検索 | 外部 MCP | ラウンド context へ注入 |

**設計（v1.3）:** イベントごと `ts` / 任意 `tsEnd` / `seq` / `eventId`（UTC）· セッション `clientTimezone`（IANA・任意）· idle TTL **7日** / max **30日**（[config/orchestration-memory.json](./config/orchestration-memory.json)）。
...
| 型定義 | [src/types/orchestration-session.ts](./src/types/orchestration-session.ts) |
...
**AS-IS:** **ラウンドループ・Hard Gate 再実行は未実装**。1 回の provider 実行（parallel / sequential）→ aggregator 1 回。`WALL_BOUNCE_MIN_PROVIDERS` デフォルトは `1`（憲法の 2+ と乖離 — Track C で enforce 予定）。
...
**To-Be（任意・ゲート付き）:** 大量 ingest 時のみ [OpenAI Batch API](./docs/OPENAI_BATCH_API_RAG.md) でチャンク要約・タグ付け — **Phase 0 では実装しない**（`batch_size` は並列数のみ）。
...
|------|---------------|---------------|
| 統一 MCP + adapter | **実装済 + G7 通過** | 日常スモーク + A-2/A-3（Track A 残） |
...
| **記憶基盤（TS-22）** | Layer A 必須 transcript · UTC イベント時刻 · idle 7d / max 30d · 単一ストア | [TECH_STACK_MEMORY_SUBSTRATE.md](./docs/decisions/TECH_STACK_MEMORY_SUBSTRATE.md) |
